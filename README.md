...
Extension of Laravel Echo Server, a NodeJs server for Laravel Echo broadcasting with Socket.io, that makes use of a multi-node setup.
...
The cli tool will help you setup a **echos-server.json** file in the root directory of your project. This file will be loaded by the server during start up. You may edit this file later on to manage the configuration of your server.

#### API Clients

The Echos Server exposes a light http API to perform broadcasting functionality. For security purposes, access to these endpoints from http referrers must be authenticated with an API id and key. This can be generated using the cli command:

``` shell
$ echos-server client:add APP_ID
```

If you run `client:add` without an app id argument, one will be generated for you. After running this command, the client id and key will be displayed and stored in the **echos-server.json** file.
...
You can remove clients with `echos-server client:remove APP_ID`
...
Edit the default configuration of the server by adding options to your **echos-server.json** file.
...
location /socket.io {
	    proxy_pass http://echos-server:6001; #could be localhost if Echo and NginX are on the same box
...
The working directory in which `echos-server` will look for the configuration file `echos-server.json` can be passed to the `start` command through the `--dir` parameter like so: `echos-server start --dir=/var/www/html/example.com/configuration`

## Subscribers
The Echos Server subscribes to incoming events with two methods: Redis & Http.

### Redis

 Your core application can use Redis to publish events to channels. The Echos Server will subscribe to those channels and broadcast those messages via socket.io.

### Http

Using Http, you can also publish events to the Echos Server in the same fashion you would with Redis by submitting a `channel` and `message` to the broadcast endpoint. You need to generate an API key as described in the [API Clients](#api-clients) section and provide the correct API key.
...
Cross domain access can be specified in the echos-server.json file by changing `allowCors` in `apiOriginAllow` to `true`. You can then set the CORS Access-Control-Allow-Origin, Access-Control-Allow-Methods as a comma separated string (GET and POST are enabled by default) and the Access-Control-Allow-Headers that the API can receive.
...
Each database driver may be configured in the **echos-server.json** file under the `databaseConfig` property. The options get passed through to the database provider, so developers are free to set these up as they wish.
...
``` json
{
  "databaseConfig" : {
    "sqlite" : {
      "databasePath": "/path/to/echos-server.sqlite"
    }
  }
}
```
...
Optionally, you can configure echos-server to publish an event on each update to a presence channel, by setting `databaseConfig.publishPresence` to `true`:

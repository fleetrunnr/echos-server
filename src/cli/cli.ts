...
export class Cli {
    ...
    configure(yargs: any): void {
        ...
        this.setupConfig(yargs.argv.config).then(options => {
            ...
            this.saveConfig(options).then(file => {
                console.log('Configuration file saved. Run ' + colors.magenta.bold('echos-server start' + (file != 'echos-server.json' ? ' --config="' + file + '"' : '')) + ' to run server.');
    ...
    getConfigFile(file: string = null, dir: string = null): string {
        const filePath = path.join(dir || '', file || 'echos-server.json');
